...
#   dist/claude-sermon-skills-all.zip      (전체 묶음 1개)
...
{
  echo "claude-sermon-skills — Distribution Manifest"
...
if [[ -z "$ONLY" ]]; then
  echo ""
  info "전체 묶음 생성 중..."
  ALL_ZIP="$DIST_DIR/claude-sermon-skills-all.zip"
  rm -f "$ALL_ZIP"

  # 임시 빌드 디렉토리
  BUILD_DIR="$DIST_DIR/.build/claude-sermon-skills"
  ...
  (cd "$DIST_DIR/.build" && zip -qr "$ALL_ZIP" "claude-sermon-skills" -x "*.DS_Store" -x "__MACOSX/*")
  rm -rf "$DIST_DIR/.build"

  size=$(du -h "$ALL_ZIP" | cut -f1)
  ok "claude-sermon-skills-all.zip ($size)"

  {
    echo ""
    echo "── Bundle ──"
    echo "  - claude-sermon-skills-all.zip ($size)"
  } >> "$MANIFEST"
fi
...
echo "    1. dist/sermon-<name>.zip 또는 claude-sermon-skills-all.zip 다운로드"
